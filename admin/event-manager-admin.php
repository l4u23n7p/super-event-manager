...
if ( ! class_exists( 'EventManager_admin' ) ) :

	/**
	 * Classe regroupant les méthodes pour l'affichage de l'administration
	 * principale du plugin
	 */
	class EventManagerAdmin {

		/**
		 * Constructeur de la classe EventManagerAdmin
		 ...
		 */
		function admin_menu() {
			if ( ! event_manager_get_setting( 'show_admin' ) ) {
				return;
			}

			$slug = 'edit.php?post_type=event';
			$cap  = event_manager_get_setting( 'capability' );
		...
		function admin_enqueue_scripts() {
			...
			wp_enqueue_style( 'event-manager-admin-design' );
		}
	}

	// initialize
	event_manager()->admin = new EventManagerAdmin();
